...
module.exports = {
    ...
    execute(message) { //index.js 에서 message값을 가져옴
        ...
        if (status.desktop) { //유저의 접속장치가 컴퓨터라면 ?
            ...
            image.push("https://cdn.discordapp.com/attachments/968751405156433973/1017597241508237520/littledeep_computer_style1.webp")
        }

        if (status.mobile) { //유저의 접속장치가 모바일이라면 ?
            devices.push("모바일") //6번줄에서 생성한 배열에 "모바일" 쑤셔넣음
            image.push("https://cdn.discordapp.com/attachments/968751405156433973/1017597749358768189/90149643---------.webp")
        }

        if (status.web) { //유저의 접속장치가 웹이라면 ?
            devices.push("웹") //6번줄에서 생성한 배열에 "웹" 쑤셔넣음
            image.push("https://cdn.discordapp.com/attachments/968751405156433973/1017598275056050176/maxresdefault.jpg")
        }

        devices = devices.join(', ') //위에 과정이 완료되면 devices값은 예시로 "[ '컴퓨터', '웹' ]" 이렇게 될것이다 이것을 "컴퓨터 , 웹" 처럼 바꿔줌

        const embed = new MessageEmbed() //임베드를 생성
            .setTitle(`${user.tag || user.user.tag}님의 접속기기`) //임베드 제목을 유저의 닉네임,태그로 설정
